...
const forbiddenTerms = ['wiler', 'supplier', 'reference', 'cost', 'sources', 'research'];
...
if (!Array.isArray(catalog.channels) || !catalog.channels.includes('fabric') || !catalog.channels.includes('karv_design')) {
...
for (const material of catalog.materials) {
  for (const key of Object.keys(material)) {
    if (!allowedKeys.has(key)) throw new Error(`${material.id ?? 'material'}: campo público não permitido: ${key}`);
  }
  if (!/^((fabric|design)-kv-[0-9]{3,})$/.test(material.id)) throw new Error(`id público inválido: ${material.id}`);
  ...
  if (material.published !== true || material.ready_for_configurator !== true) {
    throw new Error(`${material.id}: somente materiais publicados e habilitados podem entrar no contrato público`);
  }
  if (!material.compatibility?.geometry_ids?.includes('karv-chair') || material.compatibility.min_geometry_version > 2) {
  ...
  for (const [assetName, assetUrl] of Object.entries(material.assets ?? {})) {
    ...
    if (typeof assetUrl !== 'string' || !assetUrl.startsWith(`./assets/${material.id}/`) || assetUrl.includes('..') || /^https?:/i.test(assetUrl)) {
...
console.log(`Public catalog: PASS (${catalog.materials.length} materiais, sem metadata privada)`);
